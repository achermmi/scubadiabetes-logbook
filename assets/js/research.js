...
(function($) {
    ...
    $(document).ready(function() {
        var $sel = $('#sd-f-diver');
        sdResearch.divers.forEach(function(d) {
    ...
    function getSelectedYears() {
        var years = [];
        $('#sd-f-years input:checked').each(function() {
            years.push($(this).val());
        });
        return years;
    }
    ...
    function renderCharts(agg) {
        ...
        // 1) CHECKPOINT BAR
        charts.checkpoint = new Chart(document.getElementById('chart-checkpoint'), {
            type: 'bar',
            data: {
        ...
        if (agg.timeline.length > 0) {
            charts.timeline = new Chart(document.getElementById('chart-timeline'), {
                type: 'line',
                data: {
                    labels: agg.timeline.map(function(t) { return fmtDate(t.date); }),
                    datasets: [{
                        label: 'Media glicemia (mg/dl)',
                        data: agg.timeline.map(function(t) { return t.avg; }),
                        borderColor: '#1976D2',
        ...
        if (years.length > 0) {
            ...
            var yearDatasets = years.map(function(yr, i) {
                var yd = byYear[yr];
                return {
                    label: yr + ' (n=' + yd.count + ')',
            ...
            charts.yearCompare = new Chart(document.getElementById('chart-year-compare'), {
                type: 'bar',
                data: {
                    labels: ['-60 min', '-30 min', '-10 min', 'POST'],
