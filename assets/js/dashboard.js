...
(function ($) {
    ...
    $(document).ready(
        function () {
            ...
            // ============================================================
            $(document).on(
                'click', '.sd-dive-card, .sd-btn-detail', function (e) {
                    e.stopPropagation();
            ...
            function renderDetail(dive, diabetes)
            {
                ...
                if (diabetes) {
                    ...
                    if (diabetes.dive_decision) {
                        ...
                        if (diabetes.dive_decision_reason) {
                            html += ' — <span style="font-size:12px;">' + esc(diabetes.dive_decision_reason) + '</span>';
                        }
                        html += '</div>';
                    }
                }
            ...
            // CLOSE MODAL
            // ============================================================
            $('#sd-modal-close, #sd-btn-close-modal, #sd-modal-overlay').on(
                'click', function (e) {
                    if (e.target === this) {
                    ...
                    $.ajax(
                        {
                            ...
                            success: function (response) {
                                if (response.success) {
                                    ...
                                    $('.sd-dive-card[data-dive-id="' + currentDiveId + '"]').fadeOut(
                                        300, function () {
                                            $(this).remove();
                                        }
                                    );
