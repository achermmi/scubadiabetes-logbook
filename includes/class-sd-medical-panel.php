...
class SD_Medical_Panel {
    ...
    public function enqueue_assets() {
        ...
        if ( is_a( $post, 'WP_Post' ) && has_shortcode( $post->post_content, 'sd_medical_panel' ) ) {
            ...
            wp_localize_script( 'sd-medical', 'sdMedical', array(
    ...
    public function render_panel( $atts ) {
        ...
        $med_staff_users = get_users( array(
        ...
        foreach ( $all_users as $user ) {
            ...
            if ( empty( $obj->full_name ) ) $obj->full_name = $user->display_name;
            $obj->dive_count = (int) $wpdb->get_var( $wpdb->prepare(
                "SELECT COUNT(*) FROM {$db->table('dives')} WHERE user_id = %d", $user->ID
            ) );
            $obj->last_dive = $wpdb->get_var( $wpdb->prepare(
                "SELECT MAX(dive_date) FROM {$db->table('dives')} WHERE user_id = %d", $user->ID
            ) );
            $divers[] = $obj;
        }

        // Stats
        $total_divers = count( array_filter( $divers, function( $d ) { return $d->dive_count > 0; } ) );
        $diabetic_count = count( array_filter( $divers, function( $d ) { return $d->is_diabetic && $d->dive_count > 0; } ) );
    ...
    public function get_diver_data() {
        ...
        if ( ! $diver_id ) wp_send_json_error();
        ...
        if ( $is_diabetic ) {
            $profile = $wpdb->get_row( $wpdb->prepare(
                "SELECT * FROM {$db->table('diver_profiles')} WHERE user_id = %d", $diver_id
            ) );
        }
    ...
    public function save_supervision_note() {
        ...
        $wpdb->insert( $db->table( 'medical_supervision' ), array(
    ...
    public function export_research_data() {
        ...
        $filename = 'scubadiabetes-research-' . date( 'Y-m-d' ) . '.csv';
